...
Public Class CaculateService
    ...
    Public Shared Function NMRMAE(SourceList As List(Of NMRInfo), TargetList As List(Of NMRInfo)) As Decimal
        Dim q = From s In SourceList Join t In TargetList On s.Peak Equals t.Peak
                Select New With {.Item = Math.Abs(s.IntensityRel - t.IntensityRel)}
        If q.Count > 0 Then
            Dim d = q.Sum(Function(s) s.Item) / q.Count
            Return d
        Else
        ...
        End If
    End Function
    Public Shared Function NMRMSE(SourceList As List(Of NMRInfo), TargetList As List(Of NMRInfo)) As Decimal
        Dim q = From s In SourceList Join t In TargetList On s.Peak Equals t.Peak
                Select New With {.Item = (s.IntensityRel - t.IntensityRel) * (s.IntensityRel - t.IntensityRel)}
        If q.Count > 0 Then
            Dim d = q.Sum(Function(s) s.Item) / q.Count
            Return d
        Else
        ...
        End If
    End Function
    Public Shared Function NMRSim(SourceList As List(Of NMRInfo), TargetList As List(Of NMRInfo)) As Decimal
        Dim q = From s In SourceList Join t In TargetList On s.Peak Equals t.Peak
                Select New With {.Item = (s.IntensityRel - t.IntensityRel) * (s.IntensityRel - t.IntensityRel)}
        If q.Count > 0 Then
            Dim d = Math.Pow(q.Sum(Function(s) s.Item), 0.5)
